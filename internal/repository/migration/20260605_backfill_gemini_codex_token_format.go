...
func backfillGeminiCodexTokenFormatMigration(tx *gorm.DB) error {
	...
	createBackfillTableSQL := createSQLiteGeminiCodexTokenBackfillTableSQL
	hourlyBucketFilter := "hourly_bucket_start <> ''"
	dailyBucketFilter := "daily_bucket_start <> ''"
	if tx.Dialector.Name() == "postgres" {
		createBackfillTableSQL = createPostgresGeminiCodexTokenBackfillTableSQL
		hourlyBucketFilter = "hourly_bucket_start IS NOT NULL"
		dailyBucketFilter = "daily_bucket_start IS NOT NULL"
	}

	statements := []string{
		`DROP TABLE IF EXISTS temp_gemini_codex_token_backfill`,
		createBackfillTableSQL,
...
WHERE id IN (SELECT id FROM temp_gemini_codex_token_backfill)`,
		...
		fmt.Sprintf(`CREATE TEMP TABLE temp_gemini_codex_token_hourly AS
SELECT
...
GROUP BY hourly_bucket_start, api_group_key, model, auth_index, model_alias`, hourlyBucketFilter),
...
WHERE EXISTS (
	...
	WHERE t.bucket_start = usage_overview_hourly_stats.bucket_start
		...
		fmt.Sprintf(`CREATE TEMP TABLE temp_gemini_codex_token_daily AS
SELECT
...
WHERE %s
	AND apply_overview_delta = 1
GROUP BY daily_bucket_start, api_group_key, model, auth_index, model_alias`, dailyBucketFilter),
...
const createSQLiteGeminiCodexTokenBackfillTableSQL = `CREATE TEMP TABLE temp_gemini_codex_token_backfill AS
...
const createPostgresGeminiCodexTokenBackfillTableSQL = `CREATE TEMP TABLE temp_gemini_codex_token_backfill AS
WITH source_events AS (
	SELECT
		e.id,
		e.auth_type AS raw_auth_type,
		...
		CASE
			...
			ELSE COALESCE(e.model_alias, '')
		END AS model_alias,
		e.timestamp AS timestamp_value,
		COALESCE(e.input_tokens, 0) AS input_tokens,
		COALESCE(e.output_tokens, 0) AS output_tokens,
		COALESCE(e.reasoning_tokens, 0) AS reasoning_tokens,
		COALESCE(e.total_tokens, 0) AS total_tokens,
...
SELECT
	...
	model_alias,
	CASE
		WHEN timestamp_value IS NULL THEN NULL
		ELSE date_trunc('hour', timestamp_value)
	END AS hourly_bucket_start,
	CASE
		WHEN timestamp_value IS NULL THEN NULL
		ELSE date_trunc('day', timestamp_value)
	END AS daily_bucket_start,
	apply_overview_delta,
	apply_identity_delta,
	new_output_tokens,

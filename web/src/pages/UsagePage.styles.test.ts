...
const requestDetailJsonViewerSource = readFileSync(new URL('../components/usage/RequestDetailJsonViewer.tsx', import.meta.url), 'utf8')
...
const usagePageDevTabsSource = readFileSync(new URL('./usagePageDevTabs.ts', import.meta.url), 'utf8')
...
describe('UsagePage toolbar styles', () => {
  ...
  it('lets Monitoring recent request logs reuse the Request Events table and detail action', () => {
    ...
    expect(monitoringCenterSource).not.toContain('styles.requestLogTableWrapper')
    ...
    expect(sharedTableSource.indexOf("usage_stats.request_events_timestamp")).toBeLessThan(sharedTableSource.indexOf("usage_stats.model_name"))
    expect(sharedTableSource.indexOf("usage_stats.model_name")).toBeLessThan(sharedTableSource.indexOf("usage_stats.reasoning_effort"))
    expect(sharedTableSource.indexOf("usage_stats.reasoning_effort")).toBeLessThan(sharedTableSource.indexOf("usage_stats.request_events_source"))
    expect(sharedTableSource.indexOf("usage_stats.request_events_source")).toBeLessThan(sharedTableSource.indexOf("usage_stats.request_events_result"))
    expect(sharedTableSource.indexOf("usage_stats.request_events_result")).toBeLessThan(sharedTableSource.indexOf("usage_stats.time"))
    expect(sharedTableSource.indexOf("usage_stats.total_tokens")).toBeLessThan(sharedTableSource.indexOf("usage_stats.total_cost"))
    ...
    expect(monitoringCenterSource).toContain("reasoningEffort: String(log.reasoning_effort ?? '').trim() || '-'")
  ...
  it('keeps Request Event Log headers visible while the table scrolls', () => {
    expect(usagePageStyles).toMatch(/\.requestEventsTableFrame\s*\{[\s\S]*?height:\s*clamp\(560px,\s*72vh,\s*760px\);/)
    expect(usagePageStyles).toMatch(/\.requestEventsTableFrame\s*\{[\s\S]*?overflow:\s*hidden;/)
    expect(usagePageStyles).toMatch(/\.requestEventsTableFrame\s*\{[\s\S]*?border-radius:\s*\$radius-lg;/)
    expect(usagePageStyles).toMatch(/\.requestEventsTableFrame\s*\{[\s\S]*?box-shadow:/)
    expect(usagePageStyles).toMatch(/\.requestEventsTableScroll\s*\{[\s\S]*?flex:\s*1 1 auto;/)
    expect(usagePageStyles).toMatch(/\.requestEventsTableScroll\s*\{[\s\S]*?overflow:\s*auto;/)
    expect(usagePageStyles).toMatch(/\.requestEventsTableScroll\s*\{[\s\S]*?thead\s+th\s*\{[\s\S]*?position:\s*sticky;/)
    expect(usagePageStyles).toMatch(/\.requestEventsTableScroll\s*\{[\s\S]*?thead\s+th\s*\{[\s\S]*?top:\s*0;/)
    expect(usagePageStyles).toMatch(/\.requestEventsTableScroll\s*\{[\s\S]*?thead\s+th\s*\{[\s\S]*?z-index:\s*2;/)
    expect(usagePageStyles).toMatch(/\.requestEventsTableScroll\s*\{[\s\S]*?\.table\s*\{[\s\S]*?border-collapse:\s*separate;/)
    expect(usagePageStyles).toMatch(/\.requestEventsTableFooter\s*\{[\s\S]*?z-index:\s*3;/)
    expect(usagePageStyles).toMatch(/\.requestEventsTableFooter\s*\{[\s\S]*?padding:\s*14px \$spacing-lg \$spacing-lg;/)
    expect(usagePageStyles).toMatch(/\.requestEventsTableFooter\s*\{[\s\S]*?background:/)
    expect(usagePageStyles).toMatch(/\.requestEventsTableFooter\s*\{[\s\S]*?box-shadow:\s*0 -12px 24px/)
  ...
  it('renders Request Detail JSON with separated tabs and axonhub-style collapsible controls', () => {
    ...
    expect(requestEventsSource).toContain('styles.requestEventsDetailTrafficTabs')
    expect(requestDetailJsonViewerSource).toContain('styles.requestEventsDetailJsonBlock')
    expect(requestEventsSource).toContain('request_events_detail_request_headers')
    expect(requestEventsSource).toContain('request_events_detail_response_body')
    ...
    expect(usagePageStyles).toMatch(/\.requestEventsDetailPageTabs,\s*\n\.requestEventsDetailTrafficTabs\s*\{[\s\S]*?border-radius:\s*\$radius-full;/)
    expect(usagePageStyles).toMatch(/\.requestEventsDetailPageTab,\s*\n\.requestEventsDetailTrafficTab\s*\{[\s\S]*?cursor:\s*pointer;/)
    expect(usagePageStyles).toMatch(/\.requestEventsDetailJsonGrid\s*\{[^}]*flex-direction:\s*column;/)
    expect(usagePageStyles).not.toMatch(/\.requestEventsDetailJsonGrid\s*\{[^}]*grid-template-columns:/)
    expect(usagePageStyles).toMatch(/\.requestEventsDetailJsonBlock\s*\{[\s\S]*?text-transform:\s*uppercase;/)
    expect(usagePageStyles).toMatch(/\.requestEventsDetailJsonViewer\s*\{[\s\S]*?contain:\s*inline-size;/)
    expect(usagePageStyles).toMatch(/\.requestEventsDetailJsonRow\s*\{[\s\S]*?&:hover\s*\{[\s\S]*?background:/)
    expect(usagePageStyles).toMatch(/\.requestEventsDetailJsonToggle\s*\{[\s\S]*?cursor:\s*pointer;/)
    expect(usagePageStyles).toMatch(/\.requestEventsDetailJsonChildren\s*\{[\s\S]*?padding-left:\s*8px;/)
    expect(usagePageStyles).toMatch(/\.requestEventsDetailJsonParseButton,\s*\n\.requestEventsDetailJsonParsedHeader button\s*\{[\s\S]*?text-decoration:\s*underline;/)
    expect(requestEventsSource).toContain('<textarea')
    expect(requestEventsSource).toContain('wrap="off"')
    expect(requestEventsSource).not.toContain('<pre className={styles.requestEventsDetailRawLog}')
    expect(usagePageStyles).toMatch(/\.requestEventsDetailRawLog\s*\{[^}]*white-space:\s*pre;/)
    expect(usagePageStyles).toMatch(/\.requestEventsDetailRawLog\s*\{[^}]*overflow-wrap:\s*normal;/)
    expect(usagePageStyles).toMatch(/\.requestEventsDetailRawLog\s*\{[^}]*word-break:\s*normal;/)
    expect(usagePageStyles).toMatch(/\.requestEventsDetailRawLog\s*\{[^}]*resize:\s*vertical;/)
    expect(usagePageStyles).not.toMatch(/\.requestEventsDetailRawLog\s*\{[^}]*white-space:\s*pre-wrap;/)
    expect(usagePageStyles).not.toMatch(/\.requestEventsDetailRawLog\s*\{[^}]*overflow-wrap:\s*break-word;/)
    expect(usagePageStyles).not.toMatch(/\.requestEventsDetailRawLog\s*\{[^}]*word-break:\s*break-word;/)
  })
